...
class AnsuchenController extends BaseController
{
    ...
    public function showAction(Ansuchen $ansuchen): ResponseInterface
    {
        ...
        $nextuid = $this ->ansuchenRepository->getNextAnsuchenUid($uid);
        $nextAnsuchen = $this->ansuchenRepository->findByUid($nextuid);
        if ($nextAnsuchen) {
            $this->view->assign('nextAnsuchen', $nextAnsuchen);
        }
    ...
    public function certificateDownloadAction(Ansuchen $ansuchen)
    {
        $this->check($ansuchen);
        $ansuchennummer = $ansuchen->getNummer();
        $firstVersion = $this->ansuchenRepository->getFirstVersion($ansuchennummer);
        $this->view->assignMultiple([
            'ansuchen' => $ansuchen,
            'stammdaten' => $this->stammdatenRepository->getLatest(),
